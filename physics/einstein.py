...
from core.utils import central_gradient, central_laplacian, hessian_tensor, smooth_and_normalize
    ...
    for beta in betas:
        for seed in seeds:
            ...
            for idx in np.ndindex(rho.shape):
                gvec = np.array([grads[i][idx] for i in range(ndim)])
                grad2 = float(np.sum(gvec * gvec))
                lap_val = float(sum(central_laplacian(rho, axis=ax)[idx] for ax in range(ndim)))
                hloc = np.array([[hess[i, j][idx] for j in range(ndim)] for i in range(ndim)], dtype=float)
                ...
                g_anis = float(np.max(np.abs(eig_g)) / (np.min(np.abs(eig_g)) + 1e-12))

                t_eff = outer + gamma_source * np.eye(ndim) * grad2 + eta_source * np.eye(ndim) * lap_val
                eig_t = np.linalg.eigvalsh(t_eff)
                t_trace = float(np.sum(eig_t))
                t_anis = float(np.max(np.abs(eig_t)) / (np.min(np.abs(eig_t)) + 1e-12))
